...
export const requestEntropy = async (salt?: Uint8Array) =>{
	const hexSalt = toHexString(salt ?? crypto.getRandomValues(new Uint8Array(32)));
...
export const encMessage = async (message: string) => {
	...
	const encKey = _sodium.from_hex(entropyHex).subarray(0,_sodium.crypto_secretbox_KEYBYTES);
	return  `${_sodium.to_hex(salt)}.${_sodium.to_hex(nonce)}.${_sodium.crypto_secretbox_easy(message,nonce, encKey,"base64")}`
}

export const decMessage = async (cipherText: string): Promise<Uint8Array> => {
	const array = cipherText.split('.');
	if(array.length !==3) {
		throw new SnapError('Invalid backup data',SnapErrorCode.InvalidBackupData);
	}
