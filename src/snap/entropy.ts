...
export default class Entropy {
	public static requestEntropy = async (salt?: Uint8Array) => {
	...
	public static encMessage = async (message: string) => {
		const salt = crypto.getRandomValues(new Uint8Array(32));
		const nonce = _sodium.randombytes_buf(_sodium.crypto_secretbox_NONCEBYTES);
		const entropyHex = await this.requestEntropy(salt);
	...
	public static decMessage = async (cipherText: string): Promise<Uint8Array> => {
